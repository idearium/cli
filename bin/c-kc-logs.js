...
program
    .arguments('<selector>')
    .description(
        'Output logs from all Kubernetes pods. The selector defaults to `--selector svc`.'
    )
    .parse(process.argv);

const [svc] = program.args;

return Promise.all([loadState('env'), loadConfig()]).then(([env, config]) => {
    ...
    exec(
        `stern --color always --selector ${selector} --namespace ${namespace} --context ${environments[env].context}`
    );
});
