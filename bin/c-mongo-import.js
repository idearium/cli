...
loadConfig('mongo')
    .then((mongo) => {
        ...
        return spawn(`docker run -it -v ${process.cwd()}/data/${db.name}:/data/${db.name} --add-host ${localDb.host}:$(c hosts get -n ${localDb.host}) --rm mongo:latest mongorestore --drop -h ${localDb.host}:${localDb.port} -d ${localDb.name} data/${db.name}`, {
            shell: true,
            stdio: 'inherit',
        });
