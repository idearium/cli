...
program
    ...
    .description(
        'Expose services to the world via Ngrok. The <action> should be either start or stop.'
    )
    .parse(process.argv);
...
if (!['start', 'stop'].includes(action)) {
    return reportError(
        new Error('You need to provide an action. Either start or stop.'),
        program,
        true
    );
}
    ...
    .then(
        (config) =>
            new Promise((resolve, reject) => {
                ...
                exec(
                    `kubectl run ${location} --image=stefanprodan/ngrok --overrides='{"spec":{"containers":[{"name":"ngrok","image":"stefanprodan/ngrok:latest","command":["./ngrok","start","-config=/home/ngrok/.ngrok2/ngrok.yaml","--all"],"volumeMounts":[{"name":"config","mountPath":"/home/ngrok/.ngrok2/"}]}],"volumes":[{"name":"config","configMap":{"name":"ngrok"}}]}}' --namespace=${prefix} --restart=Never --image-pull-policy=IfNotPresent -i --tty --rm`,
                    (code, stdout, stderr) => {
                        if (stderr) {
