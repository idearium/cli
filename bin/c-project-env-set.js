...
program
    .arguments('<env>')
    .description(
        'Set the current environment for your project. It must be one that you have defined with c.js.'
    )
    .parse(process.argv);
...
if (!env) {
    return reportError(
        new Error('You must pass the environment argument.'),
        program
    );
}
...
return loadConfig('environments')
    .then((data) => {
        if (!data[env]) {
            return reportError(
                new Error(
                    `The '${env}' environment hasn't been defined. See https://github.com/idearium/cli#configuration`
                )
            );
        }
