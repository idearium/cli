...
program
    .arguments('<location>')
    .description(
        'Provide a Docker location and all Docker images from that location will be cleaned. Pass `all` as the location to clean all locations.'
    )
    .parse(process.argv);
...
return loadConfig()
    ...
    .then(([prefix, locations]) => {
        locations.forEach((loc) => {
            exec(
                `docker rmi -f $(docker images --filter reference=${prefix}/${loc} -q --no-trunc)`
            );
        });
    });
