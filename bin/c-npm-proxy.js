...
program
    ...
    .description(
        'Provide an NPM location, and a NPM command to run against that location. For example `c npm proxy app install -SE @idearium/cli`.'
    )
    .parse(process.argv);

if (!program.args.length) {
    return reportError(
        new Error('You need to provide an NPM location'),
        program
    );
}
...
if (program.args.length === 1) {
    return reportError(
        new Error('You need to provide an NPM command'),
        program
    );
}
...
return loadConfig('npm')
    .then((data) => {
        ...
        if (location === 'all') {
            return proxyCommands(
                Object.keys(locations).map((loc) => [
                    resolve(process.cwd(), locations[loc]),
                    npmCommand,
                ])
            );
        }
        ...
        return proxyCommand(
            resolve(process.cwd(), locations[location]),
            npmCommand
        );
    })
    .catch((err) => {
