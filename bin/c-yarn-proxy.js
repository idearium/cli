...
program
    ...
    .description(
        'Provide an NPM location, and a Yarn command to run against that location. For example `c yarn proxy app add -E @idearium/cli`.'
    )
    .parse(process.argv);
...
if (program.args.length === 1) {
    return reportError(
        new Error('You need to provide a Yarn command'),
        program
    );
}
