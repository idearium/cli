...
- Added `c project env ls`, `c project env get` and `c project env set` to help with project environment management.
- Added a `c d build` command.
- Added `c kc build`, `c kc clean`, `c kc context`, `c kc context get` and `c kc context set` commands to help with kubectrl.
